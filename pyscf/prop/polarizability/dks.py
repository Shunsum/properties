...
from .dhf import DHFPolar


class DKSPolar(DHFPolar):
    pass

dks.DKS.Polarizability = lib.class_as_method(DKSPolar)
...
if __name__ == '__main__':
    ...
    mf = mol.DKS(xc='b3lyp').run(conv_tol=1e-14)
    hcore = mf.get_hcore()
    pl = DKSPolar(mf)
    h1 = pl.get_h1()
    polar = pl.polar()
    ...
    def apply_E(E):
        mf.get_hcore = lambda *args, **kwargs: hcore + lib.einsum('x,xuv->uv', E, h1)
        mf.run(conv_tol=1e-14)
        return DKSPolar(mf).polarizability()
    print(hyperpolar)
    e1 = apply_E([ 0.0001, 0, 0])
    e2 = apply_E([-0.0001, 0, 0])
    print((e1 - e2) / 0.0002)
